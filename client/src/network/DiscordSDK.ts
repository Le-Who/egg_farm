...
const isDiscord =
  typeof window !== "undefined" && window.location.search.includes("frame_id");

/**
 * Initialize the Discord SDK — in dev mode returns mock data.
 */
export async function initDiscordSDK(): Promise<AuthResult> {
  if (isDiscord) {
 ...
 */
let cachedUser: DiscordUser | null = null;
 ...
 */
export async function getEntitlements(_skuId: string): Promise<boolean> {
  // In production: check Discord entitlements API
  console.log(
    "[DiscordSDK] Entitlement check stubbed — always returns false in dev",
  );
  return false;
}
