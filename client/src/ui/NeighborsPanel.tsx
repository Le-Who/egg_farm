...
export const NeighborsPanel: React.FC<{ isOpen: boolean; onClose: () => void }> = ({
  isOpen,
  onClose,
}) => {
  ...
  return (
    <div style={styles.overlay}>
      <div style={styles.panel}>
          ...
          <>
            ...
            {online.map((n) => (
              <div key={n.discordId} style={styles.neighborRow}>
                ...
                <button style={styles.visitBtn} onClick={() => handleVisit(n.discordId)}>
                  Visit 🏠
                </button>
              </div>
            ))}
        ...
        {offline.length > 0 && (
          <>
            <div style={{ ...styles.sectionLabel, marginTop: 16 }}>⚫ Offline ({offline.length})</div>
            {offline.map((n) => (
              <div key={n.discordId} style={{ ...styles.neighborRow, opacity: 0.5 }}>
                <span style={styles.avatar}>😴</span>
                <span style={styles.name}>{n.displayName}</span>
              </div>
            ))}
        ...
        {neighbors.length === 0 && (
          <p style={styles.empty}>No neighbors found. Join a Discord voice channel!</p>
        )}
      </div>
    </div>
  );
...
const styles: Record<string, React.CSSProperties> = {
  ...
  sectionLabel: {
    color: '#aaa', fontSize: 12, fontWeight: 700, textTransform: 'uppercase' as const,
  ...
  empty: { color: '#888', textAlign: 'center' as const, fontSize: 14, fontFamily: "'Inter', sans-serif" },
};
